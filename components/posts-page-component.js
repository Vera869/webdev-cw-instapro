...
export function renderPostsPageComponent() {
  ...
  const appHTML = getPosts.map((post, index) => {
        ...
        <div class="post-header-container">
          ...
          <div class="post-likes">
            ...
            Нравится: ${
                post.usersLikes.length > 0 ? `${post.usersLikes[post.usersLikes.length - 1].name} 
                ${post.usersLikes.length - 1 > 0 ? 'и ещё ' + (post.usersLikes.length - 1): ''}`: '0'
               }
            </p>
          </div>
          <p class="post-text">
